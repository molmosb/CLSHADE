function [encabezado, soluciones] = CLSHADE(D,cotas,MAX_NFE,corridas,H,p,rNinit,Nmin)
...
if(corridas<1)
    error('El argumento corridas debe ser mayor a 0');
end
...
display(['corridas=',num2str(corridas)]);
fprintf('\n');
display(['H=',num2str(H)]);
display(['p=',num2str(p)]);
...
nombreArchivo=['/CLSHADE_' num2str(corridas) 'Ejecuciones_' num2str(NFE) 'Eval.xlsx'];
